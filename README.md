...
기본적으로 전체상품 카테고리가 추가됨. 처음엔 네이버 스마트스토어를 목표로 제작했기 때문에 판매자가
...
원래는 전혀 계획에 없었으나 애플 스토어는 색상 옵션별로 메인이미지가 바뀌길래 급하게 만들었다.
...
실제로 등록과 수정은 거의 차이가 없다. 다만, Input에 값을 새로 넣는것이 등록이고, 이미 작성이 되어있다면 수정 일것이다.
먼저 useParams를 사용하여 상품 등록 페이지에 진입했다면 값이 감지 된다. 첫 상품등록이라면 당연히 없다.
...
처음엔 S3에서 CORS 설정을 하면 된다는 이야기가 많아 아래와 같이 작성을 했는데 전혀 작동하지 않았다.
...
해결 방법은 image.crossOrigin = "Anonymous"; 코드를 canvas로 만들기 전에 넣어주면 된다.
뭔가 찜찜한 방법인데 일단 작동은 된다. (..)
...
80포트로 들어오는 트래픽을 5800포트로 리다이렉트 시켜준다.
