...
import iconhelp from "@img/icon-help-16.svg";
import iconcheck from "@img/icon-check-16.00020358.svg";
...
function Usersign() {
  ...
  // 현재 가입 조건 실시간 확인
  useEffect(() => {
    if (isId && isId2 && isPassword && isPasswordConfirm && isName && isPhone && isEmail && isEmail2 && isAddress && Agree) {
      setSubmit(true);
    } else {
      setSubmit(false);
      return;
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      const data = await axios.post("/smartstore/user/usersign", userdata, { withCredentials: true }).then((res) => {
        if (res.data.errorEmail === "이메일중복") {
          setEmailMessage("이미 가입된 이메일 입니다.");
          setIsEmail(false);
          return;
  ...
  useEffect(() => {
    const clickOutside = (e: any) => {
      ...
      if (inputClickNumber === 8 && inputRefEmail2.current && !inputRefEmail2.current.contains(e.target)) {
        setInputClick(false);
        setInputClickNumber(0);
      }

      // 우편번호, 주소, 상세주소
      if (inputClickNumber === 10 && inputRefAddress.current && !inputRefAddress.current.contains(e.target)) {
        setInputClick(false);
        setInputClickNumber(0);
      }
      if (inputClickNumber === 11 && inputRefAddress.current && !inputRefAddress.current.contains(e.target)) {
        setInputClick(false);
        setInputClickNumber(0);
      }
      if (inputClickNumber === 12 && inputRefAddress.current && !inputRefAddress.current.contains(e.target)) {
        setInputClick(false);
        setInputClickNumber(0);
      }
    };
                        ...
                        <div className="input-item">
                          <div className="input-area">
                            <div className="input-box" ref={inputRefID}>
                              <div id={inputClickNumber === 1 ? "input-inner-active" : "input-inner"}>
                                <input
                                  type="text"
                                  name="id"
                    ...
                    <div className="signup-id-pc">
                      <div className="text">
                        <label className="title">
                          휴대전화 번호
                          <img src={iconhelp} alt=" " className="icon-help" />
                        </label>
                        <div className="input-item">
                          <div className="input-area">
                          ...
                          <div className="input-area">
                            <div className="input-box" ref={inputRefAddress}>
                              <div className="addresstop">
                                <div id={inputClick && inputClickNumber === 11 ? "input-inner-active" : "input-inner"}>
                                  <input
                                    type="text"
                                    name="address"
                                    placeholder="주소"
                              ...
                              <div id={inputClick && inputClickNumber === 12 ? "input-inner-active" : "input-inner"}>
                                <input
                                  type="text"
                                  name="address"
                                  placeholder="상세주소"
                ...
                <ul className="agree-check-list">
                  <li className="agree-check-item flex">
                    ...
                    <label className="agree-check-box" htmlFor="checkbox01">
                      ...
                      <img src={iconcheck} alt=" " className="agree-ico" />
                    </label>
